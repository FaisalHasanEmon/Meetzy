...
function Navbar() {
  ...
  return (
    <nav className="bg-blue-600 text-white p-4 fixed w-full top-0 shadow-md z-50">
      <div className="container mx-auto flex justify-between items-center">
        <Link to="/" className="text-2xl font-bold">VideoCallApp</Link>

        {/* Desktop Menu */}
        <div className="hidden md:flex space-x-6">
          <Link to="/dashboard" className="hover:text-gray-300">Dashboard</Link>
          <Link to="/call" className="hover:text-gray-300">Start Call</Link>
        </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-blue-700">
          <Link to="/dashboard" className="block py-2 px-4 hover:bg-blue-800">Dashboard</Link>
          <Link to="/call" className="block py-2 px-4 hover:bg-blue-800">Start Call</Link>
          {user ? (
            <>
              <Link to="/profile" className="block py-2 px-4 hover:bg-blue-800">Profile</Link>
              <button onClick={handleLogout} className="w-full text-left py-2 px-4 hover:bg-blue-800">Logout</button>
            </>
          ) : (
            <Link to="/login" className="block py-2 px-4 hover:bg-blue-800">Login</Link>
          )}
        </div>
      )}
    </nav>
