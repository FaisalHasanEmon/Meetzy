...
// const BACKEND_URL = 'https://meetzy-server.vercel.app' || 'http://localhost:5000';
const CallInterface = () => {
    ...
    const inputRef = useRef(null);
    const [showParticipantsList, setShowParticipantsList] = useState(false); // State to toggle participants list
    const [socketInstance, setSocketInstance] = useState(null); // Hold the socket instance
    ...
    useEffect(() => {
        ...
        setSocketInstance(socket); // Store the socket instance
    ...
    const createPeerConnection = (userId, socket) => {
        ...
        peerConnection.ontrack = (event) => {
            ...
            videoElement.className = 'w-full sm:w-1/2 lg:w-1/3 h-auto object-cover p-1 rounded-lg';
            videoElement.id = `video-${userId}`;
            // Check if the video element already exists before appending.
            ...
            if (error.name === 'NotAllowedError') {
                if (error.message.includes('Permission denied')) {
                    Swal.fire({
                        ...
                        text: 'You cancelled the screen sharing request.',
                    });
                } else if (error.message.includes('Permission is denied')) {
                     Swal.fire({
                            icon: 'warning',
                            title: 'Screen Share Blocked',
                            text: 'Screen sharing is blocked by your browser or system settings.',
                        });
                }
                else {
                    Swal.fire({
                        icon: 'error',
                        title: 'Screen Share Failed',
                        text: 'Screen sharing is not allowed.',
                    });
                }
            } else {
                Swal.fire({
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white relative">
            ...
            {connectionStatus !== 'connected' && (
                <div className="absolute top-4 left-4 bg-red-600 text-white px-3 py-1 rounded-full z-50">
            ...
            <div className="w-full h-[calc(100vh-80px)] flex items-center justify-center">
                <div className={`grid ${getGridClass()} gap-2 w-full h-full p-4`} id="video-container">
                    {/* Local Video */}
                ...
                {currentEmoji && (
                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-4xl sm:text-6xl animate-bounce z-50">
                        {currentEmoji}
                    </div>
                )}
            </div>
            ...
            {/* Sidebar for Chat Messages */}
            <div
                className={`fixed z-50 right-0 top-0 w-full sm:w-96 h-full bg-gray-800 text-white transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}
            >
                    ...
                    {showMoreOptions && (
                        <div className="absolute bottom-16 right-0 bg-gray-800 rounded-lg shadow-lg w-48">
                            <button
                                ...
                                onClick={() => {
                                    setShowParticipantsList(true); // Show participants list
                                    setShowMoreOptions(false); // Hide options
                                }}
                            >
